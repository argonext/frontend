...
import { CONTACT_METHODS, OFFICES, SOCIAL_LINKS } from "../_constants/constants"

const contactMethods = CONTACT_METHODS.map(method => ({
    ...method,
    icon: method.icon === "phone" ? Phone : method.icon === "mail" ? Mail : method.icon === "map-pin" ? MapPin : Clock,
    link: method.icon === "phone" ? `tel:${method.details[0]}` : method.icon === "mail" ? `mailto:${method.details[0]}` : "#",
    value: method.details[0],
    description: method.action,
    availability: method.details.length > 1 ? method.details[1] : "Available"
}));

const offices = OFFICES;

const socialLinks = SOCIAL_LINKS.map(link => ({
    icon: link.platform === "facebook" ? Facebook : link.platform === "instagram" ? Instagram : link.platform === "linkedin" ? Linkedin : Youtube,
